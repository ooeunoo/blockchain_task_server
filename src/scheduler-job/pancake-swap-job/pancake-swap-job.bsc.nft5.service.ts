...
@Injectable()
export class PancakeSwapBSCNFT5Service extends NFTTemplate {
  ...
  async networkPid(): Promise<BigNumber> {
    return this.context.getNFToken5TotalSupply();
  }

  async crawlNFTokens(pids: number[]): Promise<any> {
    const nfTokenInfos = await this.context.getNFToken5Infos(pids);
    ...
    const crawledNFTokens = await Promise.all(
      nfTokenInfos.map(async ({ id, tokenURI }) => {
        ...
        return {
          ...
          address: this.context.nfToken5Address,
          index: id.toString(),
          token_uri: tokenURI,
