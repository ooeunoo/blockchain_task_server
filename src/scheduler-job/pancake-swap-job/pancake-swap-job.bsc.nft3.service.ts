...
@Injectable()
export class PancakeSwapBSCNFT3Service extends NFTTemplate {
  ...
  async networkPid(): Promise<BigNumber> {
    return this.context.getNFToken3TotalSupply();
  }

  async crawlNFTokens(pids: number[]): Promise<any> {
    const nfTokenInfos = await this.context.getNFToken3Infos(pids);
    ...
    const crawledNFTokens = await Promise.all(
      nfTokenInfos.map(async ({ id, tokenURI }) => {
        ...
        return {
          ...
          address: this.context.nfToken3Address,
          index: id.toString(),
          token_uri: tokenURI,
